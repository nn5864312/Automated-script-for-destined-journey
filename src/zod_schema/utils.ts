...
export const LifeSkillRankAliases: Record<string, keyof typeof LifeSkillRankMaxMap> = {
  初级: '初级',
  初級: '初级',
  初学者: '初级',
  初學者: '初级',
...
export const LifeSkillLevelExpTable = {
  '初级 1': 100,
  '初级 2': 400,
  '初级 3': 600,
  '初级 4': 1200,
...
const parseLifeSkillLevel = (level: string) => {
  ...
  if (!match) {
    return { rank: '初级' as const, step: 1, normalized: DEFAULT_LIFE_SKILL_LEVEL };
  }

  const rank = _.trim(match[1]) as (typeof LifeSkillRankOrder)[number];
